...
public class DTOApplications {
	...
	private static final String JOB_ID = "jid";
	...
	public DTOApplications(HashMap<String, Object> hm) {
		
		if (hm.get(JOB_ID) != null) {
			this.jid = hm.get(JOB_ID).toString();
		}
		if (hm.get(APP_ID) != null) {
			this.appId = hm.get(APP_ID).toString();
		}
		if (hm.get(CANDIDATE_NAME) != null) {
			this.candidateName = hm.get(CANDIDATE_NAME).toString();
		}
		if (hm.get(EMAIL) != null) {
			this.email = hm.get(EMAIL).toString();
		}
		if (hm.get(COVER_LETTER) != null) {
			this.coverLetter = hm.get(COVER_LETTER).toString();
		}
		if (hm.get(RESUME) != null) {
			this.resume = hm.get(RESUME).toString();
		}
		if (hm.get(APP_STATUS) != null) {
			this.status = hm.get(APP_STATUS).toString();
		}
